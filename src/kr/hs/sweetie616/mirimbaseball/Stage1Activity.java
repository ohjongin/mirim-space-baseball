...
public class Stage1Activity extends ActionBarActivity implements OnClickListener{
	...
	PlayListAdapter<String> adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		resultScreen = (ListView)findViewById(R.id.list_resultScreen);
		list = new ArrayList<String>();
		adapter = new PlayListAdapter<String>(this, list);
...
//            appendResult(strResult);
        	...
        	adapter.setText(s1, s2, s3, s4, s5, s6);
        ...
        do {
            ...
            for (int j = 0; j < i; j++) { // 중복되는 값 검사.
                if (nums_com[i] == nums_com[j]) {
                    i--; break; 	//중복되는 값이 있으면 i를 빼준 후 반복문 종료
                }// if
            }// for
            i++;
        } while (i < 3); // 3회전
